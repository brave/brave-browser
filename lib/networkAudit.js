...
const networkAudit = {
  parseNetworkLog: (networkLogFile) => {
    ...
    // TODO(bsclifton) rework network audit as new browser suite since this
    // may compromise integrity on Windows.
    // See https://github.com/brave/brave-browser/issues/7281
    if (process.platform === 'win32' && !jsonContent.endsWith('}]}')) {
      // On windows netlog ends abruptly causing JSON parsing errors
      // See https://github.com/brave/brave-browser/pull/2787
      const n = jsonContent.lastIndexOf('},')
      jsonContent = jsonContent.substring(0, n) + '}]}'
    }

    let exitCode = 0
    let jsonOutput = JSON.parse(jsonContent)
