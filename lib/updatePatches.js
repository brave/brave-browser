...
const updatePatches = (options) => {
  ...
  diff = util.run('git', diffArgs, { cwd: path.join(config.projects.chrome.dir, 'third_party', 'icu') })
  fs.writeFileSync(path.join(config.projects.muon.dir, 'patches', 'third_party', 'icu', 'filter.patch'), diff.stdout)
}
