...
const test = (suite, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (suite === 'brave_unit_tests' || suite === 'brave_browser_tests') {
    util.run('ninja', ['-v', '-C', config.outputDir, "brave/test:fix_test_install_name_" + suite], config.defaultOptions)
  } else {
    util.run('ninja', ['-v', '-C', config.outputDir, suite], config.defaultOptions)
  }
