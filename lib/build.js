...
const touchOverriddenFiles = (filter) => {
  ...
  sourceFiles.forEach(file => {
    var targetOriginalFile = path.join(config.srcDir, file.slice(chromiumSrcDirLen))
    if (!fs.existsSync(targetOriginalFile)) {
      // Try to check in gen dir one more time.
      console.log('Check: ' + targetOriginalFile + ' isn\'t existed. Trying to check gen file')
      targetOriginalFile = path.join(config.outputDir, 'gen', file.slice(chromiumSrcDirLen))
    }
    ...
    if (fs.existsSync(targetOriginalFile)) {
      ...
      console.log('Check: ' + targetOriginalFile + ' isn\'t existed')
    }
  })
}
