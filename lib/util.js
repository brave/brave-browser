...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (forceReset = false, cleanup = false, braveCoreRef = null, includeChromium = true, options = {}) => {
    ...
    if (includeChromium && (forceReset || util.shouldUpdateChromium())) {
    ...
    if (includeChromium && util.shouldUpdateChromium()) {
      const rerunArgs = [...initialArgs, ...chromiumArgs, ...resetArgs]
      runGClient(args, options)
    } else {
