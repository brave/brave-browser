...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (reset = false, braveCoreRef = null, options = {}) => {
    ...
    if (!reset) {
      ...
      reset = (headSHA !== targetSHA)
    }

    if (reset) {
      if (!braveCoreRef) {
        braveCoreRef = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', '--abbrev-ref', 'HEAD']).stdout.toString()
        if (braveCoreRef === 'HEAD') {
          braveCoreRef = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', 'HEAD']).stdout.toString()
        }
      }

      if (!braveCoreRef) {
        errorLog('Could not get a valid git ref for ' + config.braveCoreDir)
        process.exit(1)
      }
      ...
      args = args.concat(['--with_tags',
                          ...
                          '--upstream',
                          '--force',
                          '--revision', 'src/brave@' + braveCoreRef,
                          '--revision', 'src@' + config.getProjectRef('chrome')])
    }

    runGClient(args, options)

    // fixup gclient_branchname -> branchname
    if (reset && braveCoreRef === config.getProjectRef('brave-core')) {
