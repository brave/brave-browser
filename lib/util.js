...
const util = {
  ...
  updateBranding: () => {
    ...
    if (config.targetOS === 'android') {
      ...
      const androidThirdPartySource = path.join(config.projects['brave-core'].dir, 'third_party', 'android_deps')
      const androidThirdPartyDest = path.join(config.srcDir, 'third_party', 'android_deps')
      ...
      console.log('copy Android third_party libs')
      androidSourceFiles = []
      if (fs.statSync(androidThirdPartySource).isDirectory()) {
        androidSourceFiles = util.walkSync(androidThirdPartySource)
      } else {
        androidSourceFiles = [androidThirdPartySource]
      }
      for (const androidSourceFile of androidSourceFiles) {
        let destinationFile = path.join(androidThirdPartyDest, path.relative(androidThirdPartySource, androidSourceFile))
        if (!fs.existsSync(destinationFile) || (util.calculateFileChecksum(androidSourceFile) != util.calculateFileChecksum(destinationFile))) {
          fs.copySync(androidSourceFile, destinationFile)
        }
      }

      // Copy ConfigAPIs.java.template to ConfigAPIs.java
      const configAPIFolderPathSrc = path.join(config.projects['brave-core'].dir, 'android', 'java', 'org', 'chromium', 'chrome', 'browser', 'ConfigAPIs.java.template')
      const configAPIFolderPathDst = path.join(config.projects['brave-core'].dir, 'android', 'java', 'org', 'chromium', 'chrome', 'browser', 'ConfigAPIs.java')
      if (!fs.existsSync(configAPIFolderPathDst)) {
        fs.copySync(configAPIFolderPathSrc, configAPIFolderPathDst)
      }
    }
  },
