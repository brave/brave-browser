...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  checkoutBraveCore: (options = {}) => {
    progressLog('Checkout brave-core into src/brave...')
    if (!fs.existsSync(config.braveCoreDir)) {
      fs.mkdirSync(config.braveCoreDir)
      util.run('git', ['-C', config.braveCoreDir, 'clone', config.braveCoreRepo, '.'])
      util.run('git', ['-C', config.braveCoreDir, 'checkout', config.getProjectVersion('brave-core')])
    }
  },

  shouldUpdateChromium: () => {
    const headSHA = util.run('git', ['-C', config.srcDir, 'rev-parse', 'HEAD']).stdout.toString().trim()
    const targetSHA = util.run('git', ['-C', config.srcDir, 'rev-parse', config.getProjectRef('chrome')]).stdout.toString().trim()
    return (targetSHA !== headSHA)
  },

  gclientSync: (forceReset = false, braveCoreRef = null, options = {}) => {
    let reset = forceReset

    // base args
    let args = ['sync', '--reset', '--nohooks']

    if (forceReset || braveCoreRef) {
      if (!braveCoreRef) {
        braveCoreRef = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', '--abbrev-ref', 'HEAD']).stdout.toString().trim()
        if (braveCoreRef === 'HEAD') {
          // get the rev hash if we're not in a branch
          braveCoreRef = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', 'HEAD']).stdout.toString().trim()
        }
      }
    ...
    // get back to the correct branch name from gclient_branchname
    const newBranch = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', '--abbrev-ref', 'HEAD']).stdout.toString().trim()
    if (newBranch.startsWith('gclient_')) {
      let prog = util.run('git', ['-C', config.braveCoreDir, 'checkout', '-b', newBranch.replace(/^gclient_/, '')], {continueOnFail: true})
      if (prog.status !== 0) {
        prog = util.run('git', ['-C', config.braveCoreDir, 'checkout', newBranch.replace(/^gclient_/, '')], {continueOnFail: true})
      }
      if (prog.status === 0) {
        util.run('git', ['-C', config.braveCoreDir, 'reset', '--hard', braveCoreRef], {continueOnFail: true})
      }
    }

    // recheck the chromium version because the brave-core branch may have changed
    if (util.shouldUpdateChromium()) {
      util.gClientSync(false, null, options)
    } else {
