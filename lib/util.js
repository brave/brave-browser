...
const util = {
  ...
  updateBranding: () => {
    ...
    const braveComponentsDir = path.join(config.projects['breeze-core'].dir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const braveAppDir = path.join(config.projects['breeze-core'].dir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const braveBrowserResourcesDir = path.join(config.projects['breeze-core'].dir, 'browser', 'resources')
    const braveAppVectorIconsDir = path.join(config.projects['breeze-core'].dir, 'vector_icons', 'chrome', 'app')
    ...
    fileMap.add([path.join(config.projects['breeze-core'].dir, 'chromium_src', 'chrome', 'installer', 'setup', 'brave_behaviors.cc'),
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.projects['breeze-core'].dir, 'win_build_output', 'midl', 'google_update')
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.projects['breeze-core'].dir
