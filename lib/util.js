...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (forceReset = false, cleanup = false, braveCoreRef = null, options = {}) => {
    ...
    if (forceReset || braveCoreRef) {
      ...
      let parsedRev = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', '--abbrev-ref', 'HEAD']).stdout.toString().trim()
      if (!braveCoreRef) {
        braveCoreRef = parsedRev
      ...
      } else if (parsedRev !== 'HEAD') {
        // this is a branch so make sure we're on the right branch before running gclient sync
        let prog = util.run('git', ['-C', config.braveCoreDir, 'reset', '--hard', 'HEAD'], {continueOnFail: true})
        if (prog.status === 0) {
          prog = util.run('git', ['-C', config.braveCoreDir, 'checkout', braveCoreRef], {continueOnFail: true})
          if (prog.status !== 0) {
            prog = util.run('git', ['-C', config.braveCoreDir, 'checkout', '-b', braveCoreRef], {continueOnFail: true})
          }
        }

        if (prog.status !== 0) {
          errorLog('Could not switch to branch: ' + braveCoreRef)
