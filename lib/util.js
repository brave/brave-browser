...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (forceReset = false, braveCoreRef = null, options = {}) => {
    ...
    if (forceReset || braveCoreRef) {
      // get the rev hash if we have it
      let parsedRef = ''
      if (!braveCoreRef) {
        parsedRef = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', 'HEAD'])
      } else {
        const revParse = util.run('git', ['-C', config.braveCoreDir, 'rev-parse', braveCoreRef], {continueOnFail: true})
        if (revParse.status === 0) {
          parsedRef = revParse.stdout.toString().trim()
        }
      }

      if (parsedRef !== '') {
        braveCoreRef = parsedRef
      }
