...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  buildTarget: (options = config.defaultOptions) => {
    ...
    if (!fs.existsSync(config.outputDir)) {
      ...
      fs.writeFileSync(path.join(config.outputDir, 'args.gn'), util.buildArgsToString(config.buildArgs(), false))
    }
  ...
  buildArgsToString: (buildArgs, inline = true) => {
    let args = ''
    ...
    return inline ? args.replace(/"/g,'\\"') : args
  },
