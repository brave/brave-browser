...
const util = {
  ...
  updateBranding: () => {
    ...
    const braveAppDir = path.join(config.projects.antimuon.dir, 'app')
    fs.copySync(path.join(braveAppDir, 'brave_strings.grd'), path.join(chromeAppDir, 'brave_strings.grd'))
    fs.copySync(path.join(braveAppDir, 'settings_brave_strings.grdp'), path.join(chromeAppDir, 'settings_brave_strings.grd'))
