...
const sanitizeInput = (cmd, args) => {
  // Basic check for suspicious shell metacharacters in the command itself
  // Note: spawnSync handles arguments safely by default, but this adds a layer of auditing.
  const suspiciousPattern = /[;&|`$()]/
  if (suspiciousPattern.test(cmd)) {
    console.warn(`[SECURITY WARNING] Suspicious characters detected in command: ${cmd}`)
  }
}
