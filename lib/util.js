...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (reset = false, options = {}) => {
    ...
    if (!reset) {
      const headSHA = util.run('git', ['-C', config.srcDir, 'rev-parse', 'HEAD'], {}).stdout.toString()
      const targetSHA = util.run('git', ['-C', config.srcDir, 'rev-parse', config.getProjectRef('chrome')], {}).stdout.toString()
