...
const getProjectRef = function(projectName) {
  ...
  const muonVersion = getNPMConfig(['projects', projectName, 'version'])

  if (projectName === 'muon') {
    if (muonVersion) {
      const chromeTag = getNPMConfig(['projects', 'chrome', 'tag'])
      return `origin/${muonVersion}+${chromeTag}`
    } else {
  ...
  if (projectName === 'browser_laptop') {
    if (muonVersion) {
      return `origin/${packages.version}+${muonVersion}`
    } else {
      return `origin/master`
    }
  }
