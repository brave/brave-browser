...
const licensing = {
  updateLicenses: () => {
    ...
    let adblockComponents = []
    fs.readdirSync(braveAdBlockListsDir).forEach(file => {
      const dirPath = path.join(braveAdBlockListsDir, file)
      if (fs.statSync(dirPath).isDirectory()) {
        adblockComponents.push(extractLicenseInfo(dirPath))
      }
    })
